...
# ## Average Total Cost Overtime
#Group the data by "Admission Date", and calculate the mean "Total Cost" for each month
...
# Create a plot of the average "Total Cost" over time
avecostplot <- ggplot(dfs[["Encounter Data1"]], aes(x = `Admission Date`, y = `Total Cost`)) +
  geom_line() +
  labs(x = "Admission Date", y = "Average Monthly Total Cost (in thousands of dollars)", title = "Average Monthly Total Cost Over Time") +
  theme_minimal() +
...
ggsave(filename = "/Users/leilamulveny/Desktop/avecostplot.png", plot = revplot, width = 10, height = 8, dpi = 300)


# Net Patient Revenue Overtime
#Group the data by "Admission Date", and calculate the mean difference between "Payments Received" and "Total Cost" for each month
dfs[["Encounter Data2"]] <- dfs[["Encounter Data"]] %>%
  mutate(`Admission Date` = floor_date(`Admission Date`, "month")) %>%
  group_by(`Admission Date`) %>%
  summarise(`Cost Difference` = mean(`Payments Received` - `Total Cost`, na.rm = TRUE))

# Filter out the data for December 2018
dfs[["Encounter Data2"]] <- dfs[["Encounter Data2"]] %>%
  filter(!(`Admission Date` == as.Date("2018-12-01")))

# Create a plot of the average difference between "Payments Received" and "Total Cost" over time
diffplot <- ggplot(dfs[["Encounter Data2"]], aes(x = `Admission Date`, y = `Cost Difference`)) +
  geom_line() +
  labs(x = "Admission Date", y = "Average Net Patient Service Revenue (in thousands)", title = "Average Net Patient Service Revenue Over Time") +
  theme_minimal() +
  theme(plot.background = element_rect(fill = "white"))
...
# Save the plot as an image
ggsave(filename = "/Users/leilamulveny/Desktop/diffplot.png", plot = diffplot, width = 10, height = 8, dpi = 300)
...
long_data <- dfs[["Encounter Data Combined"]] %>%
  ...
  Value = Value / 1000) # Change the y-axis to millions

# Create a plot with all three lines
agg_drop18_plot <- ggplot(long_data, aes(x = `Admission Date`, y = Value)) +
  geom_line() +
  facet_grid(rows = vars(Variable), scales = "free_y") +
  labs(x = "Admission Date", y = "Value (Revenue and Cost in millions of dollars, Patient Count in number of patients)", title = "Monthly Trends Over Time") +
  theme_minimal() +
  theme(plot.background = element_rect(fill = "white"),
        ...
        strip.text = element_text(size = 20)) # Increase the font size of the facet labels
...
# Average Total Cost for December 2018
...
pie_chart <- ggplot(margin_by_payor, aes(x = "", y = Percentage, fill = `Payor Type`)) +
  ...
  theme_void() +
  theme(plot.background = element_rect(fill = "white"), legend.position = "right", plot.title = element_text(size = 40)) + # Increase the font size of the title
